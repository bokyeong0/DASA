...
$(document).ready(function(){
	...
	fnMakeEvent_20201();	// 버튼 이벤트 등록
	
	//지점콤보
	cbo1_20201();
	
	//직무
	cboCommon_20201('0000000002', $('#dty_code_20201'));
	
	//초기화
	controlInit_20201();

	//리스트 조회
//	fnSelectList_20201(1);
	
});

/**
 * @함수명: fnMakeEventComponent_20201
 * @작성일: 2014. 09. 29.
 * @작성자: 김진호
 * @설명: 컴포넌트 이벤트 틍록
 * @param 
 */
function fnMakeEvent_20201(){
	
	//조회
	$("#btnSearch_20201").click(function(){
		fnSelectList_20201(1);
	});
	
	//초기화
	$("#btnClear_20201").click(function(){
		controlInit_20201();
	});
	
	//엑셀
	$("#btnExcel_20201").click(function(){
		var params ="";
		
		var y = parseInt($("#yy_mm_20201").val().substring(0,4));
		var m = parseInt($("#yy_mm_20201").val().substring(5,7));
		
		var lastDay = new Date(y, m, 0).getDate();
			
		params = "bhf_code=" + $('#om_code_20201').val();
		params+="&dty_code=" + $('#dty_code_20201').val();
		params+="&ea_yy="    + $('#yy_mm_20201').val().substring(0,4);
		params+="&ea_mm="    + $("#yy_mm_20201").val().substring(5,7);
		params+="&ea_em_no=" + $.trim($("#em_no_20201").val()),
		params+="&ea_em_nm=" + encodeURI(encodeURIComponent($('#em_nm_20201').val()));
		params+="&bhf_name=" + encodeURI(encodeURIComponent($('#om_code_20201 option:selected').text()));
		params+="&last_day=" + lastDay;
		
		var address = "/attandance/selectExcelExportEmpMonth?" + params;
		location.href = address;
	});
	
	//배치 수행
	$("#excuteBatchBtn_20201").click(function(){
...
function cbo1_20201(){
	$.ajax({
	    ...
	    success : function(data) {
			...
			$('#om_code_20201').html(listHtml);
	    }
	});
}

function fnSelectList_20201(currPg){
	
//	getInputDayLabel_20201();  //header
	
	var y = parseInt($("#yy_mm_20201").val().substring(0,4));
	var m = parseInt($("#yy_mm_20201").val().substring(5,7));
	var strEaYy = $.trim($("#yy_mm_20201").val().substring(0,4));
	var strEaMm = $.trim($("#yy_mm_20201").val().substring(5,7));
	
	var lastDay = (new Date(y ,m ,0)).getDate();
	console.log("lastDay:  "+lastDay);
	
	pg_20201 = currPg;//현재페이지 리로드
	var fnName = "fnSelectList_20201";//페이징처리 함수명
	var rowSize = 20;//표시할 행수	
	var params = {
				cm_code: login_cp_cd,
				dty_code : $.trim($("#dty_code_20201").val()),
				ea_em_no: $.trim($("#em_no_20201").val()),
		    	bhf_code: $.trim($("#om_code_20201").val()),
		    	ea_em_nm: $.trim($("#em_nm_20201").val()),
		    	ea_yy: strEaYy,
		    	ea_mm: strEaMm
			};
    	
	$.ajax({
		url : "/attandance/empMonthList",
		data:{
			  //하단 페이징 처리
			  "fnName" : fnName,
			  "params" : params,
	    ...
	    success : function(data) {
			if(data!=null && data.result.length > 0){
				...
				r_listHtml += "<tr>";
				r_listHtml += "	<td class = 'txt_center' colspan='"+ (lastDay+14) + "' >조회된 내용이 없습니다.</td>";
				r_listHtml += "</tr>";
			 
			}	
			
			$("#tbody20201_left").html(l_listHtml);
			$("#tbody20201_right").html(r_listHtml);
			$("#navi20201").html(data.navi);
			
			
			headerControl_20201();  //header
			
			if(data!=null && data.result.length > 0){
				setColor_20201();   //색상 set
			}
	    }
	});
}
...
/**
 * @함수명: getCommonCode_20201
 * @작성일: 2015. 09. 30.
 * @작성자: 최수영
 * @설명: 공통코드
 * @param 
 */
function setColor_20201(){
	...
	$.ajax({
	    ...
	    success : function(data) {
			if(data!=null && data.length > 0){
				arr20201 = new Array(data.length);
				for(var i=0; i<data.length; i++){
					var codeVo = data[i];     
					arr20201[i] = new Array(codeVo.c_ext1, codeVo.c_ext2);
				}
			}
	    }
	});
	...
	//색상set
	$('#tbody20201_right tr').each(function(index) {
		 var $tds = $(this).find('td');
		 for (var i = 0; i < $tds.length; i++){
			 var tmp = $tds.eq(i).data("code");
			 var tmp2 = $tds.eq(i).text();
			 if(tmp!=''){
				 for (var j = 0; j < arr20201.length; j++) {
					if(tmp.substring(0,2)==arr20201[j][0]){
						$tds.eq(i).css("background-color", arr20201[j][1]);
					}
				 }
			 }
			 /*else{
...
/**
 * @함수명: fnSelectList_20201
 * @작성일: 2015. 09. 18.
 * @작성자: 최수영
 * @설명: 리스트 조회
 * @param 
 */
function fnSelectListA_20201(){
	$.ajax({
		url : "/attandance/list",
	    data:{ea_em_no: $("#ea_em_no").val(),
	    	bhf_code: $("#om_code_20201").val(),
	    	ea_em_nm: $("#em_nm_20201").val(),
	    	ea_yy: $("#yy_mm_20201").val().substring(0,4),
	    	ea_mm: $("#yy_mm_20201").val().substring(5,7)},
	    type : "POST",
	    dataType : "json",
	    cache : false ,
	    success : function(data) {
...
/**
 * @함수명: controlInit_20201
 * @작성일: 2015. 09. 18
 * @작성자: 최수영
 * @설명:  화면 조회 조건 초기화
 * @param 
 */
function controlInit_20201(){
	
	$('#om_code_20201').val('');
	$('#em_nm_20201').val('');
	$('#em_no_20201').val('');
	$('#yy_mm_20201').val(fnGetToday20201());
...
function headerControl_20201(){
	var y = parseInt($("#yy_mm_20201").val().substring(0,4));
	var m = parseInt($("#yy_mm_20201").val().substring(5,7));
	var lastDay = (new Date(y, m, 0)).getDate();
	console.log("headerControl_20201 "+lastDay);
	
	if (lastDay < 28) $("#d28_20201, #w28_20201").css("display", 'none'); else $("#d28_20201, #w28_20201").css("display", '');
	if (lastDay < 29) $("#d29_20201, #w29_20201").css("display", 'none'); else $("#d29_20201, #w29_20201").css("display", '');
	if (lastDay < 30) $("#d30_20201, #w30_20201").css("display", 'none'); else $("#d30_20201, #w30_20201").css("display", '');
	if (lastDay < 31) $("#d31_20201, #w31_20201").css("display", 'none'); else $("#d31_20201, #w31_20201").css("display", '');

	getInputDayLabel_20201();
}

function getInputDayLabel_20201() {
	...
	var m = parseInt($("#yy_mm_20201").val().substring(5,7));
	var d ='';
    var $tds = $('#tbl20201_right thead tr:eq(0)').find('th');
    var fullday = ""; 
    ...
    for (var i = 0; i < $tds.length; i++){
		...
		dayLabel = week[day];
		$('#tbl20201_right thead tr:eq(1) th:nth-child('+ (i+1) +')').text(dayLabel);
		 
		if      (day==0){//일
			$('#tbl20201_right thead tr:eq(1) th:nth-child('+ (i+1) +')').css('color','red');
		}else if(day==6){//토
			$('#tbl20201_right thead tr:eq(1) th:nth-child('+ (i+1) +')').css('color','blue');
		}else{ 
			$('#tbl20201_right thead tr:eq(1) th:nth-child('+ (i+1) +')').css('color','#4c4c4c');
    	}
		
		//fnLunarToSolar20201(fullday, (i+1));
    }
}

function fnLunarToSolar20201(fullday, step){
	$.ajax({
		url : "/activity/util",
	    ...
	    success : function(data) {
	    	if(data!=null && data.result.length > 0){
				for(var i=0; i<data.result.length; i++){
					var res = data.result[i];     
					 if(res){  
						 $('#tbl20201_right thead tr:eq(1) th:nth-child('+ (step) +')').css('color','red');
 ...
 * @param 
 */
function cboCommon_20201(c_parent_code, obj){
 ...
 * @param 
 */
function fnGetBatchArr_20201(){
	
	var strEaYy = $.trim($("#yy_mm_20201").val().substring(0,4));
	var strEaMm = $.trim($("#yy_mm_20201").val().substring(5,7));
	var params = {
	    	ea_yyyymmdd: strEaYy+strEaMm+'01'
		};	
	
	$.ajax({
		url : "/attandance/excuteEmpMonthBatch",
		data:{
			  //하단 페이징 처리
			  "params" : params
			  },		
