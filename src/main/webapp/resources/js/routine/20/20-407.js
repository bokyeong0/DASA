...
var pdGrid20407;           //AUIGrid 생성 후 반환 ID
...
$(document).ready(function(){
	$("#searchSmOdrTo20407").val("4");	//차수
	
	fnGetDisPlayPdSearchComboBox();
	fnGetDisPlayPdCstmrGroupList();
	
	$("#diplySearchBtn20407").click(function(){
		fnGetDiaryPdItemGridList(1);
	});
	
	//엑셀다운로드
	$("#excelDownBtn20407").click(function(){
	...
	//Grid엑셀다운로드
	$("#excelDownGridBtn20407").click(function(){
		fnGridExcelDownload();
	});
	
	//Grid(숨김컬럼제외)엑셀다운로드
	$("#excelDownGridAllBtn20407").click(function(){
		fnExportWithoutHiddenColumns();
	});
	
	$("#searchOmCode20407").change(function(){		//지점명 클릭했을때 콤보박스 초기화
		fnGetDisPlayPdCstmrGroupList();
		var listHtml = "<option value='' selected='selected'>선택</option>";
		$("#searchMeCode20407").html(listHtml);
	});	
	
	$("#searchCgCode20407").change(function(){
		fnGetDisPlayPdEntrpsList($(this).val());    //관리업체조회
	});
	
	$("#searchEmName20407").enterSearch("fnGetDiaryPdItemGridList");	//엔터키 조회 이벤트
	
	//초기화
	$("#diplySearchResetBtn20407").click(function(){
		$("#searchOmCode20407").val("");    //지점
		$("#searchCgCode20407").val("");	//고객그룹
		$("#searchMeCode20407").val("");	//관리업체
		$("#searchEmName20407").val("");	//사원명
		$("#searchSmOdr20407").val("");	    //차수
		$("#searchAddrCode120407").val("");	//시도
		$("#searchAddrCode220407").val("");	//구군
		$("input:checkbox[id=searchCodeAt20407]").attr("checked", false); //코드포함 엑셀다운로드(고객그룹,관리업체)		
		fnGetToday20407();                  //조회 연월 set
		fnGetDiaryPdItemGridList(1);
	});
	
	fnGetCommonCodeComboBox20407("0000000107",$("#searchAddrCode120407"));
	
	$("#searchAddrCode120407").change(function(){
		fnGetCommonCodeComboBox20407($(this).val(),$("#searchAddrCode220407"));
	});
	
	//배치수행
	$("#diplyBatchBtn20407").click(function(){
	...
	//fnGetDiaryPdItemGridList(displyPdPg);	// 자동 조회 displyPdPg: 1
});//ready
...
function fnGetBatchPd(){
	...
	$.ajax({
		...
		success : function(data) {
			if(data < 1 ){
				alert("배치 수행이 실패 하였습니다.");
			}else{
				alert("배치 수행이 완료 되었습니다.");
			}
		}
	});
}//fnGetBatchPd

function fnGetCommonCodeComboBox20407(c_parent_code, target){
...
}//fnGetCommonCodeComboBox20407

function fnGetDisPlayPdEntrpsList(cg_code){
	
	$.ajax({
		url : "/store/manageEntrpsList",
		data:{"cg_code":cg_code, "om_code": $("#searchOmCode20407").val()},
	    type : "POST",
	    dataType : "json",
	    cache : false,
	    async : false,
...
function fnGetDisPlayPdCstmrGroupList(){
	$.ajax({
	    ...
	    success : function(data) {
			...
			$("#searchCgCode20407").html(listHtml);
	...
	$.ajax({
	    ...
	    success : function(data) {
			...
			if( data.result.length > 0){
				...
				$("#searchOmCode20407").html(listHtml);
	    }
	});
}//fnGetDisPlayPdSearchComboBox

function fnGetDiaryPdItemGridList(currPg){
	
	displyPdPg     = currPg;
	displayRowCnt  = $("#searchRowCnt20407").val();     //한페이지에 몇개씩 보기
	var om_code    = $("#searchOmCode20407").val();     //지점
	var cg_code    = $("#searchCgCode20407").val();	    //고객그룹
	var me_code    = $("#searchMeCode20407").val();	    //관리업체
	var em_nm      = $("#searchEmName20407").val();	    //매장명
	var sm_odr1    = $("#searchSmOdrFrom20407").val();	//차수
	var sm_odr2    = $("#searchSmOdrTo20407").val();	//차수
	var sm_area1   = $("#searchAddrCode120407").val();	//시도
	var sm_area2   = $("#searchAddrCode120407").val();	//구군
	var da_date    = $("#yy_mm_20407").val();           //연월 A20170102 kks	
	
	if(sm_odr1 > sm_odr2) {
		alert("앞의 차수는 뒤의 차수 보다 클 수는 없습니다/n검색조건을 확인 하세요~");
		return;
	}
	...
	var fnName  = "fnGetDiaryPdItemGridList"; //페이징처리 함수명
	
	// 그리드(pdGrid20407) 초기화
	pdGrid20407 = AUIGrid.destroy("#pdGrid20407", [], {});	
	
	//Server 호출
	$.ajax({
		url : "/display/pdGridList",
		type : "POST",
		data : {
			fnName : fnName,
			params : params,
		...
		success : function(data) {
			...
			$("#displayPageNavi20407").html(data.navi);   //페이징
		},
		error : function() {
			// 그리드(pdGrid20406) 초기화
			pdGrid20407 = AUIGrid.destroy("#pdGrid20407", [], {});
...
// 	                   ,groupingSummary      :{dataFields : sText, labelTexts : ["전체 합계", "부분 합계", "_$except" ]}       //그룹핑 후 합계필드를 출력하도록 설정
	...
	//실제로 #pdGrid20407 에 그리드 생성
	pdGrid20407 = AUIGrid.create("#pdGrid20407", columnLayout, auiGridProps);
	
	//그리드 ready 이벤트 바인딩
	AUIGrid.bind(pdGrid20407, "ready", function(event) {

		//최초에 정렬된 채로 그리드 출력 시킴.
		AUIGrid.setSorting(pdGrid20407, [ {
				dataField : sText, sortType : -1
			}]);
	});

	//정렬 이벤트 바인딩
	AUIGrid.bind(pdGrid20407, "sorting", function(event) {
	...
	//그룹핑  이벤트  바인딩
	AUIGrid.bind(pdGrid20407, "grouping", function(event) {
		// 보관된 소팅 정보로 그룹핑 후 소팅 실시함
		if(typeof sortingInfo != "undefined") {
			AUIGrid.setSorting(pdGrid20407, sortingInfo);
		}
	});	
	
	//필터링 이벤트 바인딩
	AUIGrid.bind(pdGrid20407, "filtering", function(event) {
		var filterInfo = event.filterCache;
	});	
	
	//푸터 객체 세팅
	AUIGrid.setFooter(pdGrid20407, footerLayout);
};//createAUIGrid
...
//	AUIGrid.removeAjaxLoader(myGridID);   //ajax 요청 후 로더 제거(AUIGrid ajax를 이용하는경우)
	...
	if(gridData.length > 0){
		AUIGrid.setGridData(pdGrid20407, gridData);  // 그리드에 데이터 세팅 data 는 JSON 을 파싱한 Array-Object 입니다.	
		
	}else{
		AUIGrid.setGridData(pdGrid20407, []);        // data가 없으면 no data 노출
	}
}//createGridData

/* fnGetToday20407() 조회 연월 set
A20170102 kks*/
function fnGetToday20407(){
...
function fnExcelDownload() {
	...
	var cg_code  = $("#searchCgCode20407").val();		//고객그룹
	var me_code  = $("#searchMeCode20407").val();		//관리업체
	var em_nm    = $("#searchEmName20407").val();		//사원명
	var sm_odr1  = $("#searchSmOdrFrom20407").val();	//차수
	var sm_odr2  = $("#searchSmOdrTo20407").val();		//차수
	var sm_area1 = $("#searchAddrCode120407").val();	//시도
	var sm_area2 = $("#searchAddrCode120407").val();	//구군
	var da_date  = $("#yy_mm_20407").val();             //연월
	
	var code_at = ""; //A20171226 k2s 체크값
	
	if("checked" ==  $("input:checkbox[id=searchCodeAt20407]").attr("checked")) { 
		code_at = "Y";
	}
...
function fnGridExcelDownload() {
	var strFileName = "관리 업체별 PD매대현황";
	
	//FileSaver.js 로 로컬 다운로드가능 여부 확인
	if(!AUIGrid.isAvailableLocalDownload(pdGrid20407)) {
		alert("로컬 다운로드 불가능한 브라우저 입니다. \n서버 사이드로 전송하여 다운로드 처리하십시오.");
		return;
	}
	
	var rowCount = AUIGrid.getRowCount(pdGrid20407);
	...
	if(rowCount >= 100000) {
		if(confirm("10만행을 엑셀 또는 CSV 로 내보내기 합니다.\n확인 클릭 - 엑셀로 내보내기\n취소 클릭 - CSV로 내보내기")) {
			AUIGrid.exportToXlsx(pdGrid20407, excelProps);
			
		} else {
			AUIGrid.exportToCsv(pdGrid20407, excelProps);
...
function fnExportWithoutHiddenColumns() {
	var omNm  = $("#searchOmCode20407 option:selected").text(); 		//지점
	
	if (omNm == "" || omNm == undefined || omNm == null || omNm == "지점") {
		omNm = "전체지점"
	}
	strFileName  = omNm + " 관리업체별 PD매대현황";
	strSheetName = "관리 업체별 PD매대현황( " +omNm+" )";
	...
	var excelProps = {fileName           : strFileName	
		             ...
		             ,exceptColumnFields : AUIGrid.getHiddenColumnDataFields(pdGrid20407) //숨김 컬럼 제외
