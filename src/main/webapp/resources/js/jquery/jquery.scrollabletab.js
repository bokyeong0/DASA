        ...
        _create: function() {
            ...
            if (o.scrollable) {
				...
				var $tabs = self.element,
					...
					$navPrev = $('<li class="ui-tabs-arrow-previous ui-state-default2" title="Previous"><a href="#"><span class="ui-icon ui-icon-carat-1-w">Previous tab</span></a></li>').prependTo($arrowsNav),
                    $navNext = $('<li class="ui-tabs-arrow-next ui-state-default2" title="Next"><a href="#"><span class="ui-icon ui-icon-carat-1-e">Next tab</span></a></li>').appendTo($arrowsNav);
					$.fn.reverse = [].reverse;
...
//							$nav.find('li').last().find('a').trigger('click');
				...
				function _addclosebutton($li)
				{
					...
					lis.each(function(){
						...
						.append(
							$('<span/>')
								.addClass('ui-corner-all ui-tabs-close')
								...
								.append(
									$('<span/>')
										...
										.click(function(e){
											...
											//alert("this : "+$lastTab.children("a").children().width());
											//alert("pull : "+pull +" nav[0] : "+ $nav[0].offsetLeft+" out :" +$lastTab.outerWidth(true));
											//d(diff);
											//if(diff>1)
											//{
											//	alert("2 : " + diff);
...
//											alert($thisLi.next('li').length);
//											if($thisLi.next('li').length==0) //Last tab is selected
//											{
//												$navNext.hide('fade');
//												_animateListToLi( 'l', $nav.find('li:last'));
//												_getHiddenOnSide($tab);
//												$nav.css('margin-right',0);
//												$thisLi.prev('li').prev('li').find('a').trigger('click');
//												$nav.find('li').last().prev().find('a').trigger('click');
//												$nav.find('li').last().find('a').trigger('click');
//											}
										})
									)
...
//					$tabs.tabs('add','/asset/asset/assetMovePg','자산이동');
					//return;
					//setTimeout(function(){
					if(_allLiWidth() > $arrowsNav.width())
					{
