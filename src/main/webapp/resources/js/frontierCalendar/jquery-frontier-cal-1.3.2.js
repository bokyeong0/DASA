...
(function($) {
		...
		*/
		...
		this.do_init = function(){
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex == 0){
					calHeaderCell.addClass("JFrontierCal-Header-Cell-Sunday-Last");
				}
			}
			...
			/*function fnHolidays(){*/
			if((currentMonthNum+1)< 10)
				txt_month= ("0"+ (currentMonthNum+1));
			else
				txt_month= (currentMonthNum+1)+"";
			//alert(txt_month);
			
			var holidayArr;
			$.ajax({
				url : "/code/codeForCal",
				data:{"c_parent_code":"0000000089", "c_ext1":txt_month},
			    type : "POST",
			    dataType : "json",
			    cache : false,
			    async : false,
			    success : function(data) {
					if(data!=null && data.length > 0){
						holidayArr = new Array(data.length);
						for(var i=0; i<data.length; i++){
							var codeVo = data[i];     
							holidayArr[i] = new Array(codeVo.c_ext1, codeVo.c_name);
							//alert(holidayArr[i][1]);
			        	}
					}else{
						holidayArr =null;
					}	
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayIndex < firstDayWkIndex){
					...
					calDayCell.setDate(dt);
					calWeekHeaderCellObj.setDate(dt);
					//calWeekHeaderCellObj.setHtml("(aaa)"+dayNum+"&nbsp;");
					...
					if(holidayArr != null){
						...
						var tmp_text = (txt_month + txt_day);
						
						for (var i= 0; i < holidayArr.length; i++) {
							//alert("tmp_text : holidayArr = " +  tmp_text + ":"+holidayArr[i][0]);
							if(tmp_text == holidayArr[i][0]){
								calWeekHeaderCellObj.setHtml(holidayArr[i][1]+ " " + dayNum+"&nbsp;");
								calWeekHeaderCellObj.setCss('color','red');
								break;
							}else{
			...
			for(var weekIndex = 2; weekIndex < numberWeekRows; weekIndex++){
				...
				for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
					...
					calDayCell.setDate(dt);						
					calWeekHeaderCellObj.setDate(dt);
					//calWeekHeaderCellObj.setHtml("(bbb)"+dayNum+"&nbsp;");
			...
			for(var dayIndex = 0; dayIndex < Calendar.dayNames.length; dayIndex++){
				...
				if(dayNum <= daysInCurrentMonth){
					...
					calDayCell.setDate(dt);						
					calWeekHeaderCellObj.setDate(dt);
					//calWeekHeaderCellObj.setHtml("(ccc)"+dayNum+"&nbsp;");
					if(holidayArr != null){
						if((currentMonthNum+1)< 10)
							txt_month= ("0"+ (currentMonthNum+1));
						else
		...
		this.buildCalendarHeader = function(){
			...
			jqyHeaderObj.css("text-align","center");
		...
		this.getWeekIndex = function(dayName){
			if(dayName.toUpperCase() == "SUN" || dayName.toUpperCase() == "SUNDAY" || dayName=="일"){
				return 0;
			}else if(dayName.toUpperCase() == "MON" || dayName.toUpperCase() == "MONDAY" || dayName=="월"){
				return 1;
			}else if(dayName.toUpperCase() == "TUE" || dayName.toUpperCase() == "TUESDAY" || dayName=="화"){
				return 2;
			}else if(dayName.toUpperCase() == "WED" || dayName.toUpperCase() == "WEDNESDAY" || dayName=="수"){
				return 3;
			}else if(dayName.toUpperCase() == "THU" || dayName.toUpperCase() == "THURSDAY" || dayName=="목"){
				return 4;
			}else if(dayName.toUpperCase() == "FRI" || dayName.toUpperCase() == "FRIDAY" || dayName=="금"){
				return 5;
			}else if(dayName.toUpperCase() == "SAT" || dayName.toUpperCase() == "SATURDAY" || dayName=="토"){
				return 6;
			}else{
				return -1
			}			
	...
	// static properties
	Calendar.dayNames = new Array("일","월","화","수","목","금","토");
	/**
